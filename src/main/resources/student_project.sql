DROP TABLE IF EXISTS jc_student_children;                 
...
CREATE TABLE jc_passport_office(                          
	...
	jc_country_struct(area_id) ON DELETE RESTRICT         
);                                                        
...
CREATE TABLE jc_student_order(
     ...
     h_passport_ofice_id integer not null,
     h_post_index char(10),
     h_street_code integer not null,
     h_bulding varchar(10) not null,
     h_extension varchar(10),
     h_apartament varchar(10),
     ...
     h_student_number varchar(30),
          ...
          w_passport_ofice_id integer not null,
          w_post_index char(10),
          w_street_code integer not null,
          w_bulding varchar(10) not null,
          w_extension varchar(10),
          w_apartament varchar(10),
           w_university_id integer not null,
           w_student_number varchar(30),
           certificate_id varchar(20) not null,
           register_oficce_id integer not null,
           marriage_date date not null,


      PRIMARY KEY(student_order_id),

      FOREIGN KEY(h_street_code)REFERENCES
         jc_street(street_code) ON DELETE RESTRICT,
      FOREIGN KEY(h_passport_ofice_id)REFERENCES
         jc_passport_office(p_office_id) ON DELETE RESTRICT,
      FOREIGN KEY(h_university_id)REFERENCES
         jc_university(university_id) ON DELETE RESTRICT,
      FOREIGN KEY(w_street_code)REFERENCES
         jc_street(street_code) ON DELETE RESTRICT,
      FOREIGN KEY(w_passport_ofice_id)REFERENCES
         jc_passport_office(p_office_id) ON DELETE RESTRICT,
      FOREIGN KEY(w_university_id)REFERENCES
         jc_university(university_id) ON DELETE RESTRICT,
      FOREIGN KEY(register_oficce_id)REFERENCES
         jc_register_office(r_office_id) ON DELETE RESTRICT
);
CREATE TABLE jc_student_children(
    ...
    c_post_index char(10),
    c_street_code integer not null,
    c_bulding varchar(10) not null,
    c_extension varchar(10),
     c_apartament varchar(10),
        PRIMARY KEY(student_child_id),
        FOREIGN KEY(c_street_code)REFERENCES
        jc_street(street_code) ON DELETE RESTRICT,
        FOREIGN KEY(c_register_office_id)REFERENCES
        jc_register_office(r_office_id) ON DELETE RESTRICT
    );

CREATE INDEX idx_student_order_status ON jc_student_order(student_order_status);
CREATE INDEX idx_student_order_id ON jc_student_children(student_order_id);
